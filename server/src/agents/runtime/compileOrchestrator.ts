 ...
 * 
 * Integrates:
 *   - ProvenanceRegistry: Statement-level audit trail (WHERE/WHEN/HOW/WHY)
 *   - HITL Approval: Section-by-section human review gates
 */
...
import {
  ProvenanceRegistry,
  createStatementProvenance,
  createStatisticProvenance,
  createTableCellProvenance,
...
export interface CompileOrchestratorInput {
  ...
  signal?: AbortSignal;
  /** Enable Human-in-the-Loop approval after each section */
  enableHITL?: boolean;
}
...
export class CompileOrchestrator {
  ...
  async compile(input: CompileOrchestratorInput): Promise<CompileOrchestratorResult> {
    ...
    try {
        ...
        if (slotKind === "NARRATIVE") {
          ...
          try {
            ...
            const isSynthesisSlot = 
              ...
              slotId.includes("benefit_risk") || slotId.includes("benefit-risk") ||
            ...
            const result = await agent.run({
              ...
              evidenceAtoms: atomsForAgent.map(a => ({
                atomId: a.atomId,
            ...
            if (result.success && result.data) {
              ...
              // ── PROVENANCE: Record statement-level traceability ──
              createStatementProvenance(
                input.psurCaseId,
                slot.slot_id,
                slot.title,
                result.data.content.substring(0, 500),
                result.data.reasoning || `Generated by ${agentName}`,
                result.data.citedAtoms,
                agentName,
              );

              // ── HITL: Pause for human approval ──
              if (input.enableHITL !== false) {
                ...
                if (decision.status === "revision_requested" && decision.feedback) {
                  // Re-run the agent with feedback injected
                  console.log(`[HITL] Revision requested for ${slot.slot_id}, re-running with feedback`);
                  ...
                  if (retryResult.success && retryResult.data) {
                    sectionData.content = retryResult.data.content;
                    sectionData.evidenceAtomIds = retryResult.data.citedAtoms;
                    sectionData.confidence = retryResult.data.confidence;

                    // Record revised provenance
                    createStatementProvenance(
                      input.psurCaseId,
                      slot.slot_id,
                      slot.title,
                      retryResult.data.content.substring(0, 500),
                      `Revised after HITL feedback: ${decision.feedback}`,
                      retryResult.data.citedAtoms,
                      agentName,
                    );
                  }
                }
              ...
              updateLiveContent(input.psurCaseId, slot.slot_id, slot.title, sectionData.content, "done");
        ...
        // ─── TABLE GENERATION ────────────────────────────────────────────────
          ...
          try {
            ...
            if (result.success && result.data) {
              ...
              // ── PROVENANCE: Record table-level traceability ──
              for (const atomId of (result.data.evidenceAtomIds || [])) {
                createStatisticProvenance(
                  input.psurCaseId,
                  slot.slot_id,
                  slot.title,
                  `Table data for ${slot.title}`,
                  `${result.data.markdown.split("\n").length} rows`,
                  { method: "AGGREGATION_COUNT" as DerivationMethod, reasoning: `Table generated by ${agentName}` },
                  [],
                  [atomId],
                  result.confidence,
                  agentName,
              ...
              if (input.enableHITL !== false) {
                const decision = await requestApproval({
                  ...
                  sectionTitle: slot.title,
                  slotKind: "TABLE",
                  generatedContent: result.data.markdown,
                  confidence: result.confidence,
                  evidenceAtomIds: result.data.evidenceAtomIds || [],
                  agentName,
                });
                ...
                if (decision.status === "revision_requested" && decision.feedback) {
                  warnings.push(`Table ${slot.slot_id}: User requested revision — "${decision.feedback}"`);
                  // Tables are data-driven so we just log the feedback for manual follow-up
                }
              }
      ...
      // Generate provenance audit report for this compilation
      const provenanceReport = ProvenanceRegistry.generateAuditReport(input.psurCaseId);
      console.log(
        `[${this.orchestratorId}] Provenance: ${provenanceReport.totalNodes} nodes tracked, ` +
        `${provenanceReport.uniqueSourceFiles} source files, ` +
        `${provenanceReport.uniqueEvidenceAtoms} atoms referenced, ` +
        `avg confidence ${(provenanceReport.averageConfidence * 100).toFixed(1)}%, ` +
        `${provenanceReport.humanReviewRequired} items flagged for review`
      );
